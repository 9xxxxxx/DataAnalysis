...
def dealfixgoods(df):
    ...
    summary = fixgoods.groupby('型号名称').agg({'维修数量': 'sum', '产品类型': lambda x:'-'.join(x)}).reset_index()
...
def dealallgoods(df):
    ...
    fixgoods,fixsummary = dealfixgoods(df)

    merged_count_df = pd.concat([exsummary, resummary,fixsummary],ignore_index=True)
    merged_count_df = merged_count_df.groupby('型号名称').agg({'换货数量':'sum', '退货数量': 'sum','维修数量':'sum','产品类型':lambda x:'-'.join(x) }).reset_index()
    merged_count_df['产品类型'] = merged_count_df['产品类型'].apply(lambda x:x.split('-')[0])

    merged_notcount_df= pd.concat([exgoods, regoods, fixgoods],ignore_index=True)
...
def separate(df):
    ...
    df_part = df_part.groupby('型号名称').agg({'换货数量':'sum', '退货数量': 'sum', '维修数量':'sum', '产品类型':lambda x:'-'.join(x) }).reset_index()
    df_product = df_product.groupby('型号名称').agg({'换货数量':'sum', '退货数量': 'sum','维修数量':'sum', '产品类型':lambda x:'-'.join(x)}).reset_index()
