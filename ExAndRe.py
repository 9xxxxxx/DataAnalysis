...
path = r"退换运营报表数据分析\源数据\销售退仓10-22.xlsx"
date = re.search(r'\d+-\d+',path)
exoutpath = fr"退换运营报表数据分析\{date[0]}换货运营汇总表.xlsx" 
returnoutpath = fr'退换运营报表数据分析\{date[0]}退货运营汇总表.xlsx'
outpath = fr'退换运营报表数据分析\{date[0]}退换货运营汇总表.xlsx'
output_file = fr'退换运营报表数据分析\{date[0]}easytofill.xlsx'
...
def main():
    makesure = input('请确认将颜色规格与商品名称做了调整：按任意键继续-------')
    dealallgoods()
